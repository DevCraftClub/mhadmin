...
if(!function_exists('dle_api_table')) {
	 ...
	 *
	 * Пользовательские таблицы DLE живут под USERPREFIX (общие при мультисайте с одной базой
	 * пользователей); остальные — под PREFIX. Список соответствует `USERPREFIX . "_*"` в ядре DLE 20.
	 */
	function dle_api_table(string $logical): string {
		...
		$prefix = isset($userTables[$logical]) ? dle_api_user_prefix() : dle_api_prefix();
...
if(!function_exists('dle_api_insert')) {
	...
	function dle_api_insert(string $logical, array $row): int {
		...
		// dle_api_table() уже даёт физическое имя с префиксом, а у соединения Cycle префикс
		// настроен (`PREFIX . '_'`) — иначе builder склеил бы `dle_dle_post`.
		$id = dle_api_db()->connection()
			->withPrefix('', false)
			->insert(dle_api_table($logical))
			->values($row)
			->run();
...
if(!function_exists('dle_api_resolve_schema')) {
	 ...
	 */
	function dle_api_resolve_schema(string $logical): \DevCraft\Dle\Schema\TableSchemaInterface {
		try {
			return SchemaRegistry::get($logical);
		} catch(\InvalidArgumentException) {
			$intro = \DevCraft\Dle\Schema\IntrospectedTableSchema::tryFromPhysical($logical);
			if($intro === null) {
...
if(!function_exists('dle_api_list')) {
	...
	function dle_api_list(
		...
		$q = \DevCraft\Dle\Fluent\TableQuery::ofSchema(dle_api_resolve_schema($logical))
			->limit($limit)
			->offset($offset);
		if($selectedColumns !== []) {
...
if(!function_exists('dle_api_query')) {
	function dle_api_query(string $logical): \DevCraft\Dle\Fluent\TableQuery {
		return \DevCraft\Dle\Fluent\TableQuery::of($logical);
	}
}
