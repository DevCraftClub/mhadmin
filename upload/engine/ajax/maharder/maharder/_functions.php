...
if (!function_exists('execute_query')) {
	function execute_query( $id, $query ) {
		...
		$query = str_ireplace(
			[ "{prefix}", "{userprefix}", "{charset}", "{engine}" ],
			[ PREFIX, USERPREFIX, COLLATE, $storage_engine ], $query
		);
		...
		if (count($db->query_errors_list)) {
			...
			foreach ($db->query_errors_list as $error) {
				$db->query(
					"INSERT INTO " . PREFIX . "_plugins_logs (plugin_id, area, error, type) values ('{$id}', '"
					. $db->safesql(htmlspecialchars($error['query'], ENT_QUOTES, $config['charset']), false)
					. "', '" . $db->safesql(htmlspecialchars($error['error'], ENT_QUOTES, $config['charset']))
					. "', 'mysql')"
				);
			}
