...
if(empty($data['name']) || empty($data['description']) || empty($data['version'])) {
	...
	foreach ($dirs as $dir) {
		if (!mkdir($dir, 0777, true) && !is_dir($dir)) {
			$mh_admin->generate_log(
				'maharder', 'new_module', serialize(sprintf('Directory "%s" was not created', $dir))
			);
			$fails['dirs'][] = [
				'dir'     => $dir,
	...
	if ((int)$data['db'] === 1) {
		$dle_connector_name               =
			$db->safesql(htmlspecialchars(trim($data['name']), ENT_QUOTES, $config['charset']));
		$dle_connector_description        = $db->safesql(
			htmlspecialchars(
				trim($data['description']), ENT_QUOTES,
				$config['charset']
			)
		);
		$dle_connector_version            = $db->safesql(
			htmlspecialchars(
				trim($data['version']), ENT_QUOTES, $config['charset']
			)
		);
		$dle_connector_dleversion         = $db->safesql(
			htmlspecialchars(
				trim($config['version_id']), ENT_QUOTES, $config['charset']
			)
		);
		...
		$icon = $db->safesql(
			clearfilepath(htmlspecialchars(
				trim($data['plugin_icon']), ENT_QUOTES,
				$config['charset']
			), [ "gif", "jpg", "jpeg", "png", "webp" ])
		);
		
		$dle_mysqlupgrade = $db->safesql(
			trim(
				"INSERT INTO {prefix}_admin_sections (name, title, descr, icon, allow_groups) VALUES ('{$data['translit']}', '{$data['name']} v{$data['version']}', '{$data['description']}', '{$icon}', '1, 2') ON DUPLICATE KEY UPDATE title = '{$data['name']} v{$data['version']}';"
			)
		);
		$dle_mysqlenable  = $dle_mysqlupgrade;
		$dle_mysqldisable =
			$db->safesql(trim("DELETE FROM {prefix}_admin_sections WHERE name = '{$data['translit']}';"));
		$dle_mysqldelete  = $dle_mysqldisable;
		
		try {
			
			$plugin =
				$db->query('SELECT * FROM ' . PREFIX . "_plugins WHERE name = '{$dle_connector_name}'");
			if ($plugin->num_rows === 0) {
				$db->query(
					'INSERT INTO ' . PREFIX
					. "_plugins (name, description, icon, version, dleversion, versioncompare, active, mysqlinstall, mysqlupgrade, mysqlenable, mysqldisable, mysqldelete, filedelete, filelist, upgradeurl, needplugin, phpinstall, phpupgrade, phpenable, phpdisable, phpdelete, notice, mnotice) VALUES ('{$dle_connector_name}', '{$dle_connector_description}', '{$icon}', '{$dle_connector_version}', '{$dle_connector_dleversion}', '{$dle_connector_dleversion_compare}', $dle_connector_active, '', '{$dle_mysqlupgrade}', '{$dle_mysqlenable}', '{$dle_mysqldisable}', '{$dle_mysqldelete}', 1, '', '', '', '', '', '', '', '', '', 0)"
				);
				$plugin_id = $db->insert_id();
				$db->query(
					"INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('"
					. $db->safesql($member_id['name']) . "', '{$_TIME}', '{$_IP}', '116', '{$name}')"
				);
