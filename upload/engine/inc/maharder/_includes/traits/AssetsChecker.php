...
trait AssetsChecker {
	...
	public function checkAssets($rewrite = false) {
		if(!is_file($this->asset_file) || $rewrite) {
			...
			file_put_contents(
				$this->asset_file, json_encode($this->assets_arr, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE)
			);
		}
		...
		$info = [
			'on_server' => count($assets), 'local' => count($files), 'missing_count' => 0, 'update_count' => 0,
			'missing'   => [], 'update' => []
		];
	...
	public function save_asset(array $data, string $file) {
		...
		if($asset_file) {
			...
			if(!file_put_contents($file_path, $asset_file, FILE_USE_INCLUDE_PATH | LOCK_EX)
			   || !is_writable(
					$file_path
				)) {
			...
			return [
				'realpath'    => realpath($file_path), 'dirname' => $pathinfo['dirname'],
				'basename'    => $pathinfo['basename'], 'filename' => $pathinfo['filename'],
				'extension'   => $pathinfo['extension'],
				'mime'        => finfo_file(finfo_open(FILEINFO_MIME_TYPE), $file_path),
				'encoding'    => finfo_file(finfo_open(FILEINFO_MIME_ENCODING), $file_path), 'size' => $stat[7],
				'size_string' => format_bytes($stat[7]), 'atime' => $stat[8], 'mtime' => $stat[9],
		...
		foreach($arr as $key => $value) {
			if(is_array($value)) {
				...
				$this->assets_arr[$dir_arr] = [
					'path' => $pathinfo['dirname'], 'file' => $dir_arr, 'hash' => hash_file('md5', $file)
				];
			}
		}
